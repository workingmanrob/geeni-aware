...
wifi_station_connect()
{
	...
	do
		...
		#### 此时所有参数应为正常的，否则连接会失败
		echo ">>>>>>>>chaojun security=$security ssid=$ssid password=$pswd"
...
case "$MODE" in
	...
	connect)
		wifi_station_connect
		if [ ! -f /etc/jffs2/wifi_on ];then
			touch /etc/jffs2/wifi_on
		fi
		return $?
		;;
	*)
