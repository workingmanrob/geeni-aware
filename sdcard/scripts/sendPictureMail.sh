...
${BODY}
"
for i in $(/mnt/bin/busybox seq 1 ${NUMBEROFPICTURES})
do
    # now loop over
...
} | /mnt/bin/busybox sendmail -H"exec /mnt/bin/openssl s_client -quiet -connect $SERVER:$PORT" -f"$FROM" -au"$AUTH" -ap"$PASS" $TO 2>/dev/null
