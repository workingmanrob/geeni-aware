...
# fails due to no existing busybox
...
enable_hardware_watchdog()
{
    ...
    /mnt/bin/busybox watchdog -t 2 -T 5 /dev/watchdog
...
sync_time()
{
    ...
    /mnt/bin/busybox ntpd -p "$ntp_srv"
}
...
*/15    *       *       *       *       /mnt/bin/busybox run-parts ${CRONPERIODIC}/15min
0       *       *       *       *       /mnt/bin/busybox run-parts ${CRONPERIODIC}/hourly
0       2       *       *       *       /mnt/bin/busybox run-parts ${CRONPERIODIC}/daily
0       3       *       *       6       /mnt/bin/busybox run-parts ${CRONPERIODIC}/weekly
0       5       1       *       *       /mnt/bin/busybox run-parts ${CRONPERIODIC}/monthly
EOF
      echo "Created cron directories and standard interval jobs" >> $LOGPATH
    fi
    /mnt/bin/busybox crond -c ${CONFIGPATH}/cron/crontabs
}
